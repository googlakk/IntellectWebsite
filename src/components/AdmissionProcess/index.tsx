  ...
  return (
    ...
    <section className="py-12 ">
      ...
      <div className="container mx-auto px-4 mt-10">
        ...
        >
          Поступление в <span className=' text-blue-600'> Intellect Pro School </span>
        </motion.h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
          {steps.map((step, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`p-6 rounded-xl cursor-pointer transition-all ${activeStep === index ? 'bg-blue-600 text-white' : 'bg-white hover:bg-gray-100'}`}
              onClick={() => setActiveStep(index)}
            >
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${activeStep === index ? 'bg-blue-500' : 'bg-blue-100 text-blue-600'}`}>
                {step.icon}
              </div>
              <h3 className="font-semibold text-lg mb-2">{step.title}</h3>
              <p className={`text-sm ${activeStep === index ? 'text-blue-100' : 'text-gray-600'}`}>{step.description}</p>
              <div className={`mt-4 h-1 w-10 rounded-full ${activeStep === index ? 'bg-white' : 'bg-blue-600'}`}></div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            <h3 className="text-xl font-bold mb-6 text-gray-900">Документы для поступления</h3>
            ...
            <p className="mt-6 text-sm text-gray-500 italic">
              P.S. Заявление на имя директора и анкета ученика выдается в приемной школы.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="mb-6 text-blue-100">Оставьте заявку, и наш администратор свяжется с вами для уточнения деталей</p>
